class Rustowl < Formula
  ...
  def install
    ...
    system "cp", "-r", `rustc --print=sysroot`.strip, "sysroot/#{active_toolchain}"

    Dir.glob("sysroot/**/*").each do |f|
      next if File.directory?(f)
      next if f =~ /\.(rlib|so|dylib|dll)$/
      File.delete(f)
    end
    
    dirs_deleted = true
    while dirs_deleted do
      dirs_deleted = false
